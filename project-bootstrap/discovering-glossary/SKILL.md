---
name: discovering-glossary
description: Use as an INLINE skill (NOT a dispatched subagent) loaded by bootstrapping-project at the glossary slot. Scans READMEs, identifiers, DB schemas, route paths, and comments to surface candidate terms; then asks the user to pick the ≤30 that matter, note aliases, and refine definitions. Writes docs/GLOSSARY.md (or the configured path) atomically.
---
...
**Announce at start:** "I'm using the discovering-glossary skill to build docs/GLOSSARY.md with you."
...
So this skill stays inline. It scans the code itself, then talks to the user about which terms are load-bearing and what they really mean. The four sibling skills (`discovering-constitution`, `discovering-architecture`, `discovering-domain-model`, `discovering-design`) follow the same pattern for the same reason.
