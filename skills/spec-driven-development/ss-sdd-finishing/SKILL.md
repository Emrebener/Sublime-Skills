...
No push, no PR, no test re-run. Push is the user's call; tests already ran (or were deliberately skipped) at Stage 9.
...
- Do NOT `rm` the state file until the merge succeeds and the branch is safely deleted. If the merge halts, state stays so Stage 11 can be re-run after the user resolves the issue.
...
**No test re-run.** Stage 9 (feature testing) was the test gate; if it ran and passed, we trust the result. If it was skipped, the user already made that call.
...
`BRANCH` is never `"main"` — Stage 7's rules don't produce that value (on `main` we auto-create the derived branch; the Case C prompt only fires off non-main branches). If it somehow is, halt and surface — that's a bug upstream, not a case to silently work around.
...
- **Do NOT `rm` the state file.** State stays so Stage 11 can be re-run after the user resolves the issue.
...
Re-running Stage 11 after a manually-completed merge:
...
| Mistake | Fix |
|---|---|
| `rm`ing the state file before the merge succeeds | NEVER — state stays so Stage 11 can be re-run after a merge failure. |
| Re-running the test suite | NEVER — Stage 9 (feature testing) was the test gate. |
...
- About to `rm` the state file before confirming the merge succeeded → STOP; state survives merge failures so Stage 11 can be re-run
...
- About to re-run the test suite → STOP; Stage 9 was the gate
