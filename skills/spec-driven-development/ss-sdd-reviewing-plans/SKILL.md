...
**Announce at start:** "I'm using the ss-sdd-reviewing-plans skill to review the plan."
...
- [NO-TDD] markers have a justification line whose category matches one of the allowed labels (see `ss-sdd-writing-plans` skill's [NO-TDD] Criteria section): `docs-only`, `config-only`, `asset-addition`, `dependency-bump`, `mechanical-rename`, `lint-only`
