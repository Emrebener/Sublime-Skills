...
**Announce at start:** "I'm using the ss-sdd-choosing-feature-branch skill to decide where the feature branch lives and commit the planning artifacts."
...
```bash
PATTERN=$("${SUBLIME_SKILLS_HOME:?SUBLIME_SKILLS_HOME is not set; see Sublime-Skills README for setup}/spec-driven-development/framework/get-config-value.sh" branching branch_pattern)
