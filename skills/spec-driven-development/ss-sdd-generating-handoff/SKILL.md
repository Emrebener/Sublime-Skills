---
name: ss-sdd-generating-handoff
...
**Announce at start:** "I'm using the ss-sdd-generating-handoff skill to write the handoff document."
...
```bash
"${SUBLIME_SKILLS_HOME:?SUBLIME_SKILLS_HOME is not set; see Sublime-Skills README for setup}"/skills/spec-driven-development/framework/validate-handoff.sh <output-path>.tmp
```
...
- About to invoke another skill (ss-sdd-writing-specs, ss-sdd-writing-plans, etc.) → STOP; you only write the handoff doc
