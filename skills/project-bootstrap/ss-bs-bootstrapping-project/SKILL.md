...
description: Use to set up a project for spec-driven development - walks the user through each convention file (constitution, architecture, glossary, domain, design) by loading the matching ss-bs-discovering-<topic> inline skill for each, then scaffolds .sublime-skills/config.yml and the supporting directories. User-invoked, not part of the SDD pipeline.
---
...
2. For each convention file (constitution → architecture → glossary → domain → design): detect → ask → load the matching `ss-bs-discovering-<topic>` skill inline → record outcome
...
8. Single commit
9. Report and direct user to `ss-sdd-coordinator`
