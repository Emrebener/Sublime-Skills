#!/usr/bin/env bash
# End-to-end smoke test for ss-web-browser-tools. Exercises every tool against
# local fixtures in an isolated session. Exits non-zero on the first
# failure. Run from the ss-web-browser-tools directory: ./test/smoke.sh
set -euo pipefail
...
until [ -f ~/.cache/ss-web-browser-tools/sessions/"$SESSION"/dialog-armed ]; do sleep 0.1; done
