...
- The `## Clarifications` section (if present — written by the `grilling-specs` skill). Each clarification is a `- Q: ... → A: ...` bullet inside a `### Session YYYY-MM-DD` subheading; grill-driven decisions are valid ADR sources when they meet the ADR-worthy criteria below
